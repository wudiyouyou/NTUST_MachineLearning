...
class BankClassifier:
    ...
    def DT_max_depth_SAI(self, arges=None, score=True, picture=None):
        '''
        DT：决策树
        :param arges: 是否进行调参
            *这里的调参，只是简单的参数调账，是调整
    ...
    def DT_min_samples_split_SAI(self, arges=None, score=True, picture=None):
        '''
        DT：决策树
        :param arges: 是否进行调参
            *这里的调参，只是简单的参数调账，是调整
    ...
    def DT_min_samples_leaf_SAI(self, arges=None, score=True, picture=None):
        '''
        DT：决策树
        :param arges: 是否进行调参
            *这里的调参，只是简单的参数调账，是调整
...
print("| - name ：\t%s\t , score ：\t%.5f\t" % ("DT_max_depth_SAI         ", modelData.DT_max_depth_SAI()))
print("| - name ：\t%s\t , score ：\t%.5f\t" % ("DT_min_samples_split_SAI ", modelData.DT_min_samples_split_SAI()))
print("| - name ：\t%s\t , score ：\t%.5f\t" % ("DT_min_samples_leaf_SAI  ", modelData.DT_min_samples_leaf_SAI()))
# print("| - 模型名称：\t%s\t , 模型评分：\t%.5f\t" % ("临近取样", modelData.KNN()))
# print("| - 模型名称：\t%s\t , 模型评分：\t%.5f\t" % ("逻辑回归", modelData.LG()))
# print("| - 模型名称：\t%s\t , 模型评分：\t%.5f\t" % ("支持向量机", modelData.SVC()))
print("| - " + "-" * 30 + "\n| - 调整参数：")
# print("| - 模型名称：\t%s\t , 模型评分：\t%.5f\t" % ("随机森林", modelData.RF(arges=True,picture="updata_rf")))
print("| - name ：\t%s\t , score ：\t%.5f\t" % ("DT_max_depth_SAI         ", modelData.DT_max_depth_SAI(arges=True,picture="_max_depth_SAI")))
print("| - name ：\t%s\t , score ：\t%.5f\t" % ("DT_min_samples_split_SAI ", modelData.DT_min_samples_split_SAI(arges=True,picture="__min_samples_spli_SAIt")))
print("| - name ：\t%s\t , score ：\t%.5f\t" % ("DT_min_samples_leaf_SAI  ", modelData.DT_min_samples_leaf_SAI(arges=True,picture="__min_samples_leaf_SAI")))
